...
export const requestChatSession = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (existingSession) {
      ...
      // Also update the associated request if exists
      if (existingSession.meta?.requestId) {
        await ChatRequest.updateOne(
    ...
    const astrologer = await User.findOne({
      ...
      .select("fullName phone photo chatRate isOnline")
      .session(session);
    ...
    if (!astrologer.isOnline) {
      throw new ApiError(400, "Astrologer is offline");
    }
    ...
    const ratePerMinute = astrologer.chatRate || astroProfile.ratepermin || 10;

    // -------------------------------
    // Get user details
    // -------------------------------
    const user = await User.findById(userId)
      .select("fullName gender placeOfBirth dateOfBirth timeOfBirth phone photo")
      .lean();
    ...
    let chat = await Chat.findOne({
      isGroupChat: false,
      participants: { $all: [userId, astrologerId], $size: 2 }
    }).session(session);
    ...
    const [chatSession] = await ChatSession.create(
      [{
        ...
        expiresAt,
        meta: {
          requestId: request.requestId,  // Store the requestId string
          request_Id: request._id,       // Store the ObjectId
          chatType
        }
      }],
      { session }
    ...
    await ChatRequest.updateOne(
      ...
      {
        $set: {
          sessionId: chatSession._id  // Link to session ObjectId
        }
      },
      { session }
    );
    ...
    // -------------------------------
    // SYSTEM GENERATED MESSAGE - ONLY FOR NEW CHAT
    // -------------------------------
    let systemMessage = null;

    if (isNewChat) {
      const systemMessageText = `Dear ${astrologer.fullName},
...
Time of Birth: ${user.timeOfBirth || "N/A"}`.trim();
      ...
      const [newSystemMessage] = await Message.create(
        [{
          ...
          systemMetadata: {
            action: "user_details_shared",
            targetUsers: [astrologerId],
            performedBy: userId
          }
        }],
        { session }
    ...
    let lastMsgId = null;
    if (userMsg) {
      lastMsgId = userMsg._id;
    } else if (systemMessage) {
      lastMsgId = systemMessage._id;
    }
    ...
    if (typeof notifyAstrologerAboutRequest === 'function') {
      await notifyAstrologerAboutRequest(req, astrologerId, {
        requestId: request.requestId,
        sessionId: chatSession.sessionId,
        userId,
...
export const getAstrologerSessions = async (req, res) => {
  try {
    ...
    // In your backend ChatSession model, ensure these statuses are supported:
    const sessionStatuses = [
      'REQUESTED', // Call requested by user
      'ACCEPTED', // Astrologer accepted the call
      'ACTIVE', // Call is currently active (ongoing)
      'ONGOING', // Same as ACTIVE (for consistency)
      'PAUSED', // Call is paused
      'COMPLETED', // Call completed successfully
      'MISSED', // Astrologer missed the call
      'REJECTED', // Astrologer rejected the call
      'CANCELLED', // User cancelled the call
      'EXPIRED', // Call request expired
      'FAILED', // Call failed due to technical issues
      'AUTO_ENDED', // Call ended automatically (time limit, etc.)
    ];
    ...
    if (status && status !== 'ALL') {
      if (status === 'ACTIVE_SESSIONS') {
        filter.status = {
          $in: ['REQUESTED', 'ACCEPTED', 'ACTIVE', 'ONGOING', 'PAUSED'],
        };
      } else if (status === 'COMPLETED_SESSIONS') {
        filter.status = 'COMPLETED';
      } else if (status === 'MISSED_SESSIONS') {
        filter.status = 'MISSED';
      } else if (status === 'EXPIRED_SESSIONS') {
...
export async function sendNotification({
  ...
  try {
    ...
    console.log(
      `Notification sent → ${user.fullName || userId} [${type}]`,
      response
    );
