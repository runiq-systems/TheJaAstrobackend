/*
  models/astrotalkSchemas.js
  Complete Ready-to-Use AstroTalk Wallet & Billing System Schemas
*/
...
const PlatformConfigSchema = new Schema({
    ...
    type: { type: String, enum: ['NUMBER', 'STRING', 'BOOLEAN', 'OBJECT', 'ARRAY'], required: true },
    description: { type: String, default: null },
    category: { type: String, enum: ['COMMISSION', 'TAX', 'WALLET', 'SESSION', 'PAYMENT', 'GENERAL'], default: 'GENERAL' },
...
const TaxConfigSchema = new Schema({
    ...
    taxType: { type: String, enum: ['GST', 'VAT', 'IGST', 'SGST', 'CGST', 'TDS'], required: true },
...
const CommissionRuleSchema = new Schema({
    ...
    description: { type: String, default: null },
    conditions: {
        astrologerTier: [{ type: String, enum: ['BASIC', 'SILVER', 'GOLD', 'PLATINUM'] }],
        sessionType: [{ type: String, enum: ['CALL', 'CHAT', 'LIVE'] }],
        userType: [{ type: String, enum: ['NEW', 'REGULAR', 'VIP'] }],
        timeRange: { from: { type: String, default: null }, to: { type: String, default: null } },
        daysOfWeek: [{ type: Number, min: 0, max: 6 }],
        minSessionDuration: { type: Number, default: 0 },
        maxSessionDuration: { type: Number, default: null }
    },
    calculationType: { type: String, enum: ['PERCENTAGE', 'FIXED_AMOUNT', 'SLAB_BASED', 'HYBRID'], default: 'PERCENTAGE' },
...
const CommissionOverrideSchema = new Schema({
    targetType: { type: String, enum: ['ASTROLOGER', 'ASTROLOGER_TIER', 'SESSION_TYPE', 'GLOBAL'], required: true },
    targetId: { type: Schema.Types.ObjectId, default: null },
    targetTier: { type: String, enum: ['BASIC', 'SILVER', 'GOLD', 'PLATINUM'], default: null },
    targetSessionType: { type: String, enum: ['CALL', 'CHAT', 'LIVE'], default: null },
    baseRule: { type: Schema.Types.ObjectId, ref: 'CommissionRule', required: true },
    overrideType: { type: String, enum: ['PERCENTAGE_CHANGE', 'FIXED_AMOUNT', 'ABSOLUTE_PERCENTAGE'], required: true },
...
const CommissionAuditSchema = new Schema({
    action: { type: String, enum: ['RULE_CREATE', 'RULE_UPDATE', 'RULE_DELETE', 'OVERRIDE_CREATE', 'OVERRIDE_UPDATE', 'OVERRIDE_DELETE', 'BULK_UPDATE', 'COMMISSION_CALCULATION', 'ADJUSTMENT'], required: true },
...
const CommissionBatchUpdateSchema = new Schema({
    ...
    description: { type: String, default: null },
    criteria: {
        astrologerTiers: [{ type: String, enum: ['BASIC', 'SILVER', 'GOLD', 'PLATINUM'] }],
    ...
    updateType: { type: String, enum: ['PERCENTAGE_CHANGE', 'ABSOLUTE_PERCENTAGE', 'FIXED_AMOUNT'], required: true },
    ...
    status: { type: String, enum: ['PENDING', 'PROCESSING', 'COMPLETED', 'FAILED', 'PARTIALLY_COMPLETED'], default: 'PENDING' },
...
const WalletSchema = new Schema({
    ...
    status: { type: String, enum: ['ACTIVE', 'BLOCKED', 'SUSPENDED', 'CLOSED'], default: 'ACTIVE', index: true },
    tier: { type: String, enum: ['REGULAR', 'PREMIUM', 'VIP'], default: 'REGULAR' },
    kycStatus: { type: String, enum: ['PENDING', 'VERIFIED', 'REJECTED', 'NOT_REQUIRED'], default: 'NOT_REQUIRED' },
...
const TransactionSchema = new Schema({
    ...
    entityType: { type: String, enum: ['USER', 'ASTROLOGER', 'PLATFORM', 'ADMIN'], required: true },
    entityId: { type: Schema.Types.ObjectId, required: true },
    type: { type: String, enum: ['CREDIT', 'DEBIT'], required: true },
    category: {
        type: String,
        enum: ['RECHARGE', 'SESSION_DEDUCTION', 'RESERVE', 'UNRESERVE', 'REFUND', 'BONUS', 'ADMIN_ADJUSTMENT', 'COMMISSION', 'PAYOUT', 'REVERSAL', 'TAX', 'CHARGEBACK', 'SETTLEMENT'],
        required: true,
        index: true
    },
    ...
    status: { type: String, enum: ['PENDING', 'PROCESSING', 'SUCCESS', 'FAILED', 'CANCELLED', 'REVERSED'], default: 'PENDING', index: true },
...
const SessionRateConfigSchema = new Schema({
    ...
    sessionType: { type: String, enum: ['CALL', 'CHAT', 'LIVE'], required: true },
...
const ReservationSchema = new Schema({
    ...
    sessionType: { type: String, enum: ['CALL', 'CHAT', 'LIVE'], required: true },
    rateConfigId: { type: Schema.Types.ObjectId, ref: 'SessionRateConfig', required: true },
    ratePerMinute: { type: Number, required: true },
    currency: { type: String, default: 'INR' },
    ...
    commissionDetails: {
        ...
        appliedOverrideId: { type: Schema.Types.ObjectId, ref: 'CommissionOverride', default: null },
        finalCommissionPercent: { type: Number, required: true },
        ...
        adjustedBy: { type: Schema.Types.ObjectId, ref: 'Admin', default: null },
    ...
    status: { type: String, enum: ['INITIATED', 'RESERVED', 'ONGOING', 'PAUSED', 'SETTLING', 'SETTLED', 'CANCELLED', 'FAILED', 'EXPIRED'], default: 'INITIATED', index: true },
...
const CouponSchema = new Schema({
    ...
    name: { type: String, required: true },
    description: { type: String, default: null },
    discountType: { type: String, enum: ['PERCENTAGE', 'FLAT_AMOUNT', 'BONUS_PERCENTAGE', 'CASHBACK', 'FREE_MINUTES', 'FIXED_PRICE'], required: true },
...
const CouponUsageSchema = new Schema({
    ...
    userId: { type: Schema.Types.ObjectId, ref: 'User', required: true, index: true },
    reservationId: { type: Schema.Types.ObjectId, ref: 'Reservation', default: null },
    rechargeId: { type: Schema.Types.ObjectId, ref: 'RechargeHistory', default: null },
    usedAt: { type: Date, default: Date.now, index: true },
    discountAmount: { type: Number, default: 0 },
    cartValue: { type: Number, default: 0 },
    status: { type: String, enum: ['APPLIED', 'USED', 'REVOKED', 'EXPIRED'], default: 'APPLIED' },
    meta: { type: Schema.Types.Mixed, default: {} }
}, { timestamps: true });
...
const RechargeHistorySchema = new Schema({
    ...
    status: { type: String, enum: ['INITIATED', 'PENDING', 'SUCCESS', 'FAILED', 'CANCELLED', 'REFUNDED', 'PARTIALLY_REFUNDED'], default: 'INITIATED', index: true },
    processedAt: { type: Date, default: null },
    completedAt: { type: Date, default: null },
    meta: { type: Schema.Types.Mixed, default: {} }
}, { timestamps: true });
...
const PayoutSchema = new Schema({
    ...
    method: { type: String, enum: ['BANK_TRANSFER', 'UPI', 'PAYTM', 'PAYPAL', 'CARD'], required: true },
    payoutAccount: { type: Schema.Types.ObjectId, ref: 'PayoutAccount', required: true },
    status: { type: String, enum: ['REQUESTED', 'APPROVED', 'PROCESSING', 'SUCCESS', 'FAILED', 'CANCELLED', 'ON_HOLD'], default: 'REQUESTED', index: true },
...
const PayoutAccountSchema = new Schema({
    astrologerId: { type: Schema.Types.ObjectId, ref: 'Astrologer', required: true, index: true },
    accountType: { type: String, enum: ['BANK', 'UPI', 'PAYTM', 'PAYPAL'], required: true },
...
const WalletAuditSchema = new Schema({
    ...
    action: { type: String, enum: ['BALANCE_UPDATE', 'STATUS_CHANGE', 'KYC_UPDATE', 'MANUAL_ADJUSTMENT', 'BLOCK', 'UNBLOCK'], required: true },
    ...
    performedBy: { type: String, enum: ['SYSTEM', 'USER', 'ADMIN', 'ASTROLOGER'], required: true },
...
const TaxInvoiceSchema = new Schema({
    ...
    entityType: { type: String, enum: ['USER', 'ASTROLOGER', 'PLATFORM'], required: true },
    ...
    status: { type: String, enum: ['DRAFT', 'GENERATED', 'SENT', 'PAID', 'CANCELLED'], default: 'DRAFT' },
    meta: { type: Schema.Types.Mixed, default: {} }
}, { timestamps: true });
...
export const CommissionOverride = getModel('CommissionOverride', CommissionOverrideSchema);
export const CommissionAudit = getModel('CommissionAudit', CommissionAuditSchema);
export const CommissionBatchUpdate = getModel('CommissionBatchUpdate', CommissionBatchUpdateSchema);
...
export const SessionRateConfig = getModel('SessionRateConfig', SessionRateConfigSchema);
...
export const RechargeHistory = getModel('RechargeHistory', RechargeHistorySchema);
...
export const PlatformEarnings = getModel('PlatformEarnings', PlatformEarningsSchema);
...
export const calculateCommission = async (astrologerId, sessionType, sessionAmount, sessionDetails = {}) => {
    ...
    const finalCommission = override ? override.finalCommissionPercent : baseCommission;
