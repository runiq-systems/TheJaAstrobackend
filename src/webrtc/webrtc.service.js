...
export class WebRTCService {
  ...
  setupSocketHandlers() {
    this.io.on("connection", (socket) => {
      ...
      socket.on(
        "toggleScreenShare",
        this.handleToggleScreenShare.bind(this, socket)
      );
  ...
  async handleCall(socket, { callerId, receiverId, callType = "AUDIO" }) {
    ...
    try {
      ...
      logger.info(`[📞 CALL_SETUP_COMPLETE] Key: ${callKey}`, {
        callRecordId: callRecord._id,
      });
    } catch (error) {
  ...
  async handleAcceptCall(socket, { receiverId, callerId, callRecordId }) {
    try {
      ...
      const sentToCaller = this.emitToUser(
        callerId,
        "callAccepted",
        payloadForCaller
      );
      if (!sentToCaller)
        logger.warn(
          `[EMIT_WARN] callAccepted NOT delivered to caller ${callerId}.`
        );

      const sentToReceiver = this.emitToUser(
        receiverId,
        "callAccepted",
        payloadForReceiver
      );
      if (!sentToReceiver)
        logger.warn(
          `[EMIT_WARN] callAccepted NOT delivered to receiver ${receiverId}.`
        );
  ...
  async handleEndCall(
    ...
    try {
      ...
      if (callRecord.userId && callRecord.astrologerId) {
        const callerData =
          callRecord.userId._id.toString() === callerId
            ? callRecord.userId
            : callRecord.astrologerId;
        const receiverData =
          callRecord.userId._id.toString() === receiverId
            ? callRecord.userId
            : callRecord.astrologerId;
  ...
  async handleCancelCall(socket, { callerId, receiverId, callRecordId }) {
    ...
    try {
      ...
      logger.info(
        `[CALL_CANCEL] ${callerId} cancelling call to ${receiverId}`,
        {
          callRecordId,
          socketId: socket.id,
        }
  ...
  async handleIceCandidate(socket, { candidate, callerId, receiverId }) {
    try {
      ...
      for (const socketId of receiverSockets) {
        ...
        console.log(
          `ICE candidate forwarded to ${receiverId} via socket ${socketId}`
        );
      }
    } catch (error) {
  ...
  flushBufferedIce(callKey, targetUserId) {
    ...
    logger.debug("[ICE] Flushed buffered candidates", {
      callKey,
      count: buf.length,
    });
  }
  ...
  async notifyReceiver(callerId, receiverId, callType, callRecord) {
    ...
    logger.debug(`[RECEIVER_NOTIFIED] Receiver ${receiverId} notified`, {
      callRecordId: callRecord._id,
    });
  }
  ...
  isUserInCall(userId) {
    return (
      this.activeCalls.has(userId) ||
      Array.from(this.pendingCalls.values()).some(
  ...
  cleanupPendingCallsBySocket(socketId) {
    for (const [callKey, callData] of this.pendingCalls.entries()) {
      if (callData.socketId === socketId) {
        ...
        this.clearCallTimeout(callKey);
        logger.debug(
          `[CLEANUP] Removed pending call ${callKey} for socket ${socketId}`
        );
      }
    }
  }
  ...
  handleToggleVideo(socket, { callRecordId, enabled, userId }) {
    try {
      ...
      if (call) {
        const otherUserId =
          call.userId.toString() === userId ? call.astrologerId : call.userId;
...
export async function sendCallNotification({
  ...
  try {
    ...
    logger.info(
      `✅ Call notification sent (${type}) → ${targetUserId} (${response})`
    );
  } catch (error) {
